module PagesHelper
  def arrow_left_options
    ...
    {class: 'arrow page left', alt: t(:previous_page), title: title, data: {target: @page.previous.number}}
  end

  def arrow_right_options
    title = [t(:page), @page.next.number].join(' ')
    {class: 'arrow page right', alt: t(:next_page), title: title, data: {target: @page.next.number}}
  end
